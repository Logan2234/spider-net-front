<script lang="ts">
  ...
  const disabledStyle = 'disabled:cursor-not-allowed disabled:opacity-50 disabled:bg-dark-gray/15';
...
<div class="my-2 flex flex-col {disabled ? 'opacity-50' : ''} {containerClass}">
  {#if label}
    <label
      for={uid}
      class="w-fit pb-1 {required ? "after:ml-0.5 after:text-red-500 after:content-['*']" : ''}">
      {label}
    </label>
  {/if}
